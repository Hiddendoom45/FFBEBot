...
public class Info extends CommandGenerics implements Command{
	...
	public void action(String[] args, MessageReceivedEvent event) {
		...
		embed.addField(new Field("Last Launch",new SimpleDateFormat("[yyyy-MM-dd-HH:mm]").format(new Date(Settings.ID))+" "+Calendar.getInstance().getTimeZone().getDisplayName(false, TimeZone.SHORT),true));
		embed.addField(new Field("Memory",(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())/(1024*1024)+"MB/"+Runtime.getRuntime().totalMemory()/(1024*1024)+"MB",true));
